...
timepoint=$(date +"%H:%M")
echo "$timepoint    **Generating ACT files...**" >> /app/log/Dwipreproc_${timestamp_initial}.txt

5ttgen fsl $anat act.nii.gz -nocrop
...
tcksift2 -act act.nii.gz streamlines_iFOD2.tck dwi_fod.nii.gz sift_weights_iFOD2.txt -force 
...
tcksift2 -act act.nii.gz streamlines_FACT.tck dwi_fod.nii.gz sift_weights_FACT.txt -force
tck2connectome -zero_diagonal -symmetric -assignment_radial_search 5 \
